...
class TestViews(ApplicationLayerTest):
    ...
    gradebook_part = {'Name': u'Quizzes', 'order': 1,
                      'MimeType': 'application/vnd.nextthought.gradebookpart'}

    gradebook_entry = {'Name': u'Quiz1', 'order': 2,
                       'AssignmentId': 'utag:nextthought.com,2011-10:NextThought-gradebook-CLC3403',
                       'MimeType': 'application/vnd.nextthought.gradebookentry'}

    grade = {'username': u'sjohnson@nextthought.com', 'grade': 85, 'NTIID': None,
    ...
    def test_gradebook_delete(self):
        ...
        href = self.require_link_href_with_rel(res.json_body['Items'][0],
                                               'CourseInstance')
    ...
    def test_policy_views(self):
        ...
        self.testapp.get(policy_view_path, status=404)

        # A non-instructor should not be able to set the policy
        self.testapp.post_json(policy_view_path,
                               policy, status=403)
