...
class SubmittedAssignmentHistoryGetView(AbstractAuthenticatedView,
    ...
    _BATCH_LINK_DROP_PARAMS = BatchingUtilsMixin._BATCH_LINK_DROP_PARAMS + \
        ('batchAroundCreator',)
    ...
    def _make_force_placeholder_usernames(self, sorted_usernames, sorted_reverse):
        ...
        """
        __traceback_info__ = sorted_usernames, sorted_reverse
        ...
        if batch_start >= len(sorted_usernames):
            ...
            # after the batch. Note that this is fine even if end_idx is
            # greater than len
    ...
    def _do_sort_realname(self, filter_usernames, sort_reverse):
        ...
        place_holder = self._make_force_placeholder_usernames(
            filter_usernames, sort_reverse)
    ...
    def __sort_usernames_by_submission(self, filter_usernames, sort_reverse=False, key=None):
        ...
        users_without_grades = {
            x for x in filter_usernames if x not in users_with_grades
        }
    ...
    def _do_sort_dateSubmitted(self, filter_usernames, sort_reverse):
        # XXX: This used to be true, but now we allow deleting individual
        # grades:
    ...
    def __call__(self):
        ...
        everyone_usernames = {
            x.Principal.username.lower() for x in enrollments.iter_enrollments()
        ...
        if filter_names:
            if     'LegacyEnrollmentStatusForCredit' in filter_names \
                ...
                if 'LegacyEnrollmentStatusForCredit' in filter_names:
                    instructors_ids = {
                        x.id.lower() for x in course.instructors
                    }
        ...
        if username_search_term:
            policy = component.getAdapter(
                self.remoteUser, IIntIdUserSearchPolicy, name='comprehensive')
            matched_unames = [
                x.username.lower() for x in policy.query(username_search_term.lower(),
                                                         provided=IUser.providedBy)
            ]
            filter_usernames = {
                x for x in filter_usernames if x in matched_unames
            }
            result['FilteredTotalItemCount'] = len(filter_usernames)
        ...
        if sort_name:
            items_factory = list
            try:
                items_iter = getattr(self, '_do_sort_' +
                                     sort_name)(filter_usernames, sort_reverse)
